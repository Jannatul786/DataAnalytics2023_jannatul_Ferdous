...
# The variances are different for each of the column variables, 1st one is 165.03 and 2nd one is 0.16
...
standaraziedCaravan <- scale(Caravan[,-86]) # when we use -86 it will not include the 86th column.
var(standaraziedCaravan[,1])
var(standaraziedCaravan[,2])
var(standaraziedCaravan[,3])

# test set
test_index <- 1:1000
test_data <- standaraziedCaravan[test_index,]
test_data
test_purchase <- purchase[test_index]
test_purchase

# train set
train_data <- standaraziedCaravan[-test_index,]
train_data
...
print(error_df)
ggplot(data = error_df, aes(k_values, error_rate)) + geom_point(col = "red") +
